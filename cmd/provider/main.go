...
func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "RabbitMq support for Crossplane.").DefaultEnvars()
	...
	if *enableChangeLogs {
		...
		kingpin.FatalIfError(err, "failed to create change logs client connection in %s", *changelogsSocketPath)
